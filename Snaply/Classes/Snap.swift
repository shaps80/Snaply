...
public final class Snap: NSObject, UIScrollViewDelegate {
   ...
   */
  public func scrollTo(nearestLocationForOffset contentOffset: CGPoint) {
    let contentOffset = contentOffsetForTargetContentOffset(contentOffset)
  ...
  public func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
    ...
    let direction = snappingDirection()
    let offset = contentOffsetForTargetContentOffset(targetContentOffset.pointee)
   ...
   */
  private func contentOffsetForTargetContentOffset(_ offset: CGPoint) -> CGPoint {
    let direction = snappingDirection()
    var next: CGFloat = 0
    var prev: CGFloat = 0
   ...
   */
  public func snappingDirection() -> SnapDirection {
  ...
  public func snappingEdge() -> SnapEdge {
    return snapEdge
  }
  ...
  public func setSnapLocations(_ locations: [CGFloat], realignImmediately: Bool) {
    ...
    scrollTo(nearestLocationForOffset: offset)
  }
  ...
  /**
   ...
   - parameter realignImmediately: If true, the scrollView will automatically re-align to the nearest location based on its current offset.
   */
  public func setSnapEdge(_ edge: SnapEdge, realignImmediately: Bool) {
    self.snapEdge = edge
    guard realignImmediately else { return }
    scrollTo(nearestLocationForOffset: scrollView.contentOffset)
  }
  ...
  /**
   ...
   - parameter realignImmediately: If true, the scrollView will automatically re-align to the nearest location based on its current offset.
   */
  public func setSnapDirection(_ direction: SnapDirection, realignImmediately: Bool) {
    self.snapDirection = direction
    guard realignImmediately else { return }
    scrollTo(nearestLocationForOffset: scrollView.contentOffset)
  }
  ...
  private func maximumValue() -> CGFloat {
    switch snapEdge {
    case .min:
      return snappingDirection() == .horizontal
